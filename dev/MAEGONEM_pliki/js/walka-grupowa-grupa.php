...
$potwor = mysql_fetch_array(mysql_query("select * from mob where id = ".$_POST['id']." and grupa = ".$_POST['id2']." and mapa = ".$postac['mapa']." and (x = ".($postac['x'] - 1)." or x = ".($postac['x'] + 1)." or x = ".$postac['x'].") and (y = ".($postac['y'] - 1)." or y = ".($postac['y'] + 1)." or y = ".$postac['y'].") and respawn <= ".$czas_ogolny." limit 1"));
...
// KOD G£ÓWNY //   
  ...
  $ogolne_sa = 0;
  $mob = mysql_fetch_array(mysql_query("select * from mob where grupa = ".$_POST['id2']." and respawn <= ".$czas_ogolny." order by id asc limit 1"));
  if(empty($mob)) break;
  if(!empty($mob)){
  ...
  if($mob['zycie'] <= 0){
    ...
    if(($paczka['szansa'] * $ust[drop]) >= $szansa){
                  ...
                  } elseif($roznicalvl > $ust[di]) { $ajax_txt .= "Zbyt duza roznica poziomow, Przedmiot zniszczony"; $i-=1;
  ...
  while($sa[postac] < $sa[mob]){
  $lista_mob = mysql_query("select * from mob where grupa = ".$_POST['id2']." and respawn <= ".$czas_ogolny." order by id asc");
  ...
  mysql_query("update mob set zycie = zycie_max where grupa = ".$mob['grupa']." and grupa > 0");
