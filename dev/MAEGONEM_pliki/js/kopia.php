...
// KOD G£ÓWNY //   
  ...
  while($postac['zycie'] > 0 && $tura[walka] < 125){  
   ...
   if($potwor['grupa'] == 0){
          $lista_mob = mysql_query("select * from mob where id = ".$_POST['id']." and respawn <= ".$czas_ogolny." ");
   } else {
          $lista_mob = mysql_query("select * from mob where grupa = ".$potwor['grupa']." and respawn <= ".$czas_ogolny." ");
   }
      // III CZÊŒÆ - ATAK GRACZA//
  while($sa[postac] >= $sa[mob]){
  if($potwor['grupa'] > 0) $ilosc_mob2 = mysql_num_rows(mysql_query("select * from mob where grupa = ".$potwor['grupa']." and respawn <= ".$czas_ogolny." "));   
  else $ilosc_mob2 = mysql_num_rows(mysql_query("select * from mob where id = ".$_POST['id']." and respawn <= ".$czas_ogolny." "));
  if($potwor['grupa'] == 0) $lista_mob2 = mysql_query("select * from mob where id = ".$_POST['id']." and respawn <= ".$czas_ogolny." ");
  else $lista_mob2 = mysql_query("select * from mob where grupa = ".$potwor['grupa']." and respawn <= ".$czas_ogolny." ");
      $ogolne_sa = 0;
  if($potwor['grupa'] == 0) $mob = mysql_fetch_array(mysql_query("select * from mob where id = ".$_POST['id']." and mapa = ".$postac['mapa']." and (x = ".($postac['x'] - 1)." or x = ".($postac['x'] + 1)." or x = ".$postac['x'].") and (y = ".($postac['y'] - 1)." or y = ".($postac['y'] + 1)." or y = ".$postac['y'].") and respawn <= ".$czas_ogolny." limit 1"));
  else $mob = mysql_fetch_array(mysql_query("select * from mob where grupa = ".$potwor['grupa']." and respawn <= ".$czas_ogolny." order by id asc, poziom asc limit 1"));
  ...
  if($mob['zycie'] <= 0){
    ...
    mysql_query("update postac set exp = exp + 1 where id = ".$postac['id']." limit 1");
    $ajax_txt .= "Pokonales ".$mob['nazwa']." i zdobyles ".$mob['exp']."";
