...
$zalozony = 0;
  ...
  } elseif($dane['typ'] == 'Konsupcyjne' && $postac['zajety_walka'] == 0){
  ...
  } elseif($dane['typ'] == 'Zwoj' && $postac['zajety_walka'] == 0){
            if($dane['ilosc'] > 0){
                mysql_query("update przedmiot_postac set ilosc = ilosc - 1 where id = ".$dane['id']." and ilosc > 0 limit 1");
                $dane['ilosc'] -= 1;
            if($dane['ilosc'] <= 0) mysql_query("delete from przedmiot_postac where id = ".$dane['id']." limit 1");
            } 
            mysql_query("update postac set mapa = ".$dane['sila'].", x = ".$dane['zrecznosc'].", y = ".$dane['intelekt']." where id = ".$postac['id']." limit 1");
            $ajax_txt = "teleport";
  } elseif($dane['typ'] == 'Doswiadczenie' && $postac['zajety_walka'] == 0){
            if($dane['ilosc'] > 0){
                mysql_query("update przedmiot_postac set ilosc = ilosc - 1 where id = ".$dane['id']." and ilosc > 0 limit 1");
                $dane['ilosc'] -= 1;
            } 
            if($dane['ilosc'] <= 0) mysql_query("delete from przedmiot_postac where id = ".$dane['id']." limit 1");
            mysql_query("update postac set exp = exp + ".$dane['zycie']." where id = ".$postac['id']." limit 1");
  } elseif($dane['typ'] == 'Stroje' && $postac['zajety_walka'] == 0){
            if($dane['ilosc'] > 0){
                mysql_query("update przedmiot_postac set ilosc = ilosc - 1 where id = ".$dane['id']." and ilosc > 0 limit 1");
                $dane['ilosc'] -= 1;
            if($dane['ilosc'] <= 0) mysql_query("delete from przedmiot_postac where id = ".$dane['id']." limit 1");
            } 
            mysql_query("update postac set obrazek = '".$dane['opis']."' where id = ".$postac['id']." limit 1");
            $ajax_txt = "teleport";
